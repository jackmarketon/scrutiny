...
fn load_plan(plan_path: String) -> Result<PlanData, String> {
    let content =
        fs::read_to_string(&plan_path).map_err(|e| format!("Failed to read plan: {}", e))?;
...
fn save_feedback(
    ...
    let original =
        fs::read_to_string(&plan_path).map_err(|e| format!("Failed to read original: {}", e))?;
    ...
    fs::write(&feedback_path, json).map_err(|e| format!("Failed to write feedback: {}", e))?;
